...
def extract_pe_metadata(filepath: str, original_filename: str) -> dict:
    ...
    try:
        ...
        if pe.FILE_HEADER.Machine == 0x014C: meta["machine_type"] = "x86 (32-bit)"
        elif pe.FILE_HEADER.Machine == 0x8664: meta["machine_type"] = "x64 (64-bit)"
                    ...
                    if hasattr(file_info, "StringTable"):
                        for st in file_info.StringTable:
                            for key, val in st.entries.items():
                                ...
                                # Map keys correctly
                                if s_key == "CompanyName": meta["company_name"] = s_val
                                elif s_key == "FileDescription": meta["file_description"] = s_val
                                elif s_key == "FileVersion": meta["file_version"] = s_val
                                elif s_key == "ProductName": meta["product_name"] = s_val
                                elif s_key == "ProductVersion": meta["product_version"] = s_val
                                elif s_key == "LegalCopyright": meta["copyright"] = s_val

        # Smart Labeling
        if pe.is_driver() or meta["file_extension"] == ".sys": meta["file_type"] = "System Driver (SYS)"
        elif meta["file_extension"] == ".cpl": meta["file_type"] = "Control Panel Applet (CPL)"
        elif pe.is_dll(): meta["file_type"] = "Dynamic Link Library (DLL)"
        else: meta["file_type"] = "Executable (EXE)"
...
def process_files(args):
    ...
    files = [f for f in os.listdir(input_dir) if f.lower().endswith((".exe", ".dll", ".sys", ".cpl"))]
    
    if not files:
        logger.info(f"Scanning directory: {os.path.abspath(input_dir)}")
        logger.warning("No matching files found. Check your input directory or file extensions.")
        return
